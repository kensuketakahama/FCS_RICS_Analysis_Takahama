...
3.  **解析設定 (Config & Preprocessing):**
    ...
    - **Trace Analysis Range (s):** 解析範囲の設定です。上のintensityの図を見て、どの範囲を解析範囲として用いるべきかを決定して下さい。
    - **ACF Calculation Mode:** データをいくつかのセグメントに分けて、それぞれのACFを計算し、平均値を取る機能です。従って、この機能をonにして、segmentを1より大きな値にすると各プロットにエラーバーが現れます。通常は "Log Binning" をONにすることを推奨します。全データを見たい場合はOFFにします。
