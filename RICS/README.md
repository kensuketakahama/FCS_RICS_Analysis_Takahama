...
- `Hist Config` : 拡散係数ヒートマップに対して拡散係数のヒストグラムを作成する機能です。Binsでヒストグラムの本数、Limit Rangeで表示する拡散係数の範囲を指定することができます。Show Current Histで現在選択しているcsvファイルに対してヒストグラムを作成します。
- `Batch Analysis` : Run Batch Histograms, Run Batch Heatmapsで選択したディレクトリ内全てのcsvファイルに対して、ヒストグラムとヒートマップを作成します。Renameを選択すると、既存のヒストグラム、ヒートマップに対して上書き保存を回避できます。

### 5.3. Display Settings

- `表示パラメータの選択` : Auto Scaleは取得された拡散係数のうち、下から何パーセントまでを表示させるかを選択します。この選択を外すとMax Dによる閾値設定になります。
- `Scale Bar` : スケールバーの描画を行います。撮影時のピクセルサイズと出力したいバーのサイズ、そしてスケールバーの色を選択してください。
- `Ref Box Sixe` : windowの描画を行います。解析に用いたwindow sizeを入力して、描画したい位置をヒートマップ上でクリックしてください。
- `Mesure Length` : ヒートマップ上で2点間の距離を測ることができます。

### 5.4. ROI Settings

Load ROIと多角形によるROIの選択ができます。また、Calculate Mean Dを選択することで、ROI内外の平均拡散係数を計算することができます。また、Export ROI csvを選択することで、選択したROI以外の領域を非数とした拡散係数ヒートマップ(csv形式)を出力することができます。

## 6. Multi-File Analysis

選択したディレクトクトリ内にあるcsvをヒートマップとヒストグラムにして、並列表示します。PDF出力を行うと、1枚目と2枚目にヒートマップ、ヒストグラムのサマリーが出力されます。
...
**A.** `Step` の値を大きくすることを検討してください（例: 4や8）。しかし、その場合解像度が下がることには留意して下さい。`Step=1` は全画素計算となるため時間がかかります。また、Live Plotの描画負荷を抑えるため、画面更新は一定間隔に制限されています。

# Appendix: 技術詳細

## 1. RICS フィッティングモデル式

本アプリケーションでは、3次元自由拡散モデルに走査型顕微鏡の空間走査特性を組み込んだ標準的なRICS式を使用しています。
